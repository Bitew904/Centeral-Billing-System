...
die('Could not connect: ' . mysql_error());
	...
	if(mysql_num_rows($sql) < 1) {
            echo " ";
			echo "<script>alert('account id not found ,please enter account correctly!.');window.location.href='insertmoney.php';</script>";
		   exit();
    }
...
die('Error: ' . mysql_error());
	...
	$s=mysql_query("select * from bank where accno='$accno' ");
	while($row=mysql_fetch_array($s)){
		$accno=$row['accno'];
		$bamountofbirr=$row['amountofbirr'];
		
	}
	
	$amount1=$bamountofbirr+$amountofbirr;
$s=("UPDATE bank SET amountofbirr='$amount1' WHERE accno='$accno'");
if (!mysql_query($s,$p))
{
die('Error: ' . mysql_error());
